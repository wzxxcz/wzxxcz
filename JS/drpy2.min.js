...
function fixAdM3u8(m3u8_text, m3u8_url, ad_remove) {
    ...
    while (i < m3_len) {
        ...
        if (mi.startsWith("#EXTINF")) {
            ...
            i += 2
        } else if (mi.startsWith("#EXT-}catch(e){console.log(`第${cnt+1}次验证码提交失败:${e.message}`);if(cnt+1>=OCR_RETRY){cookie=""}}cnt+=1}return cookie}function setItem(k,v){local.set(RKEY,k,v);console.log(`规则${RKEY}设置${k} => ${v}`)}function getmurl of murls) {
